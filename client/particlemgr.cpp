...
particlemgr::particlemgr(ResourceLoader *resources)
{
...
void particlemgr::emit(int num, ResourceLoader::texType tex, partcb fnc, partinit initfnc, const vec *pos, const vec *vel)
{
	for (int i = 0; i < num; i++)
	{
		particle_data p;
    ...
    if (err != 0) {
	 ...
	 for (int i = 0; i < MAX_TEXTURES; i ++) {
        ...
        if (t != NULL)
		{
			...
			{
				if (p.tex == static_cast<ResourceLoader::texType>(i))
				{
